...
- [x] **ASELF-02**: Single "Recommended Next Practice Area" card surfaces `Associate.recommendedArea` with a one-line "why" and a "Not now" dismiss. Dismiss persists for 7 days via localStorage.
- [x] **ASELF-03**: Readiness-goal progress bar shows current weighted readiness vs cohort threshold. Associates do NOT see cohort-mate names (aggregate-only privacy default).
...
- [x] **CLEANUP-03**: Remove env vars `APP_PASSWORD`, `ASSOCIATE_SESSION_SECRET`, `ENABLE_ASSOCIATE_AUTH` from all envs + docs. CLAUDE.md + PROJECT.md updated to reflect Supabase-only auth.
