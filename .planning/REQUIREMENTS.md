...
- [x] **AUTH-03**: Associate auth coexists with trainer auth. Middleware uses identity enumeration (`trainer | associate | anonymous`) with separate cookie names. No cross-contamination between `/trainer` routes and associate-protected routes.
- [x] **AUTH-04**: Associate profile page `/associate/[slug]` requires auth. Either the matching associate session OR an active trainer session grants access. Mismatched associates (different slug) return 403. Unauthenticated requests redirect to PIN entry.
