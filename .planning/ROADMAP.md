...
- **999.4 Shell Edge Cases** — Three codex-flagged regressions deferred to protect the trainer shell state. (a) `/history` and `/question-banks` use bare TopBar without AppShell; the `md:hidden` on the wordmark hides it on desktop there, leaving no home link. Fix: conditional wordmark visibility when no desktop sidebar owns it, or migrate those pages into AppShell. (b) `AssociateShell`'s sidebar wordmark hardcodes `/trainer`; on associate pages it should route to `/associate/<slug>/dashboard`. Fix: thread a `homeHref` prop (or role) into SectionSidebar. (c) Collapsed Settings icon only opens the accordion state — sidebar stays collapsed so the accordion is invisible. Fix: wire an `onExpandSidebar` callback so the collapsed Settings icon expands the sidebar first. Estimate: 1 small phase, ~2-3h.
